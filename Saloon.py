...
stock_message_id = 1104795401929887878
accounting_message_id = 1104795405088211115
salary_message_id = 1104806650369540227
...
class SalesView(discord.ui.View):
    ...
    async def buy_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        await interaction.followup.send("Veuillez choisir une catégorie :", view=CategoryView(Purchase=True),
                                        ephemeral=True)


class CategoryView(discord.ui.View):
    def __init__(self, Purchase=False):
        super().__init__()
    ...
    async def handle_category(self, interaction, category):
        ...
        select_menu = ItemSelect(placeholder=f"Choisissez un {category} à {'acheter' if self.Purchase else 'vendre'}", options=options,
                                 user=interaction.user, Purchase=self.Purchase)

        await interaction.response.send_message("Veuillez choisir un article à {'acheter' if self.Purchase else 'vendre'} :",
...
class PurchaseView(discord.ui.View):
    ...
    @discord.ui.button(style=discord.ButtonStyle.secondary, label="Achat", custom_id="purchase_button")
    async def purchase_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        await interaction.followup.send("Veuillez choisir une catégorie :", view=CategoryView(Purchase=True), ephemeral=True)
